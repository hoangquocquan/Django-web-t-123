"""Generate IIS W3C compatible logs from production-like traffic.

Script nay tao IIS W3C training log, CSV evidence va metadata STAGING_SIMULATION.
No khong doc production, khong shutdown, khong disable routes va khong bypass
validator.
"""
...
try:
    from scripts.phase11_1_6_5_4_generate_production_like_traffic import DEFAULT_TRAFFIC_OUTPUT
    from scripts.phase11_1_6_5_4_generate_production_like_traffic import generate_traffic
except ModuleNotFoundError:
    from phase11_1_6_5_4_generate_production_like_traffic import DEFAULT_TRAFFIC_OUTPUT
...
DEFAULT_LOG_PATH = DEFAULT_EVIDENCE_DIR / "input" / "iis_logs" / "u_ex_training.log"
...
def load_or_create_traffic(path=None, requests=1200, clients=5, days=7):
    """Doc traffic JSON, neu chua co thi tao traffic simulation moi."""
    traffic_path = Path(path or DEFAULT_TRAFFIC_OUTPUT)
    if not traffic_path.exists():
        return generate_traffic(output_path=traffic_path, requests=requests, clients=clients, days=days), traffic_path
    return json.loads(traffic_path.read_text(encoding="utf-8")), traffic_path


def parse_timestamp(value):
    """Chuyen ISO timestamp thanh datetime."""
    return datetime.fromisoformat(str(value).replace("Z", "+00:00"))


def write_iis_log(requests, log_path=None):
    """Ghi IIS W3C compatible training log."""
...
def write_csv(requests, csv_path=None, source_path=None):
    """Ghi CSV evidence theo format collector IIS."""
...
def write_metadata(requests, metadata_path=None):
    """Ghi metadata collection cho STAGING_SIMULATION."""
    ...
    metadata = {
        ...
        "notes": "Production-like training evidence. Not real production IIS traffic.",
    }
    path.write_text(json.dumps(metadata, indent=2, ensure_ascii=False), encoding="utf-8")
    return metadata


def generate_iis_logs(traffic_path=None, log_path=None, csv_path=None, metadata_path=None, requests=1200, clients=5, days=7):
    """Tao IIS W3C log, CSV evidence va collection metadata."""
    traffic_payload, source_traffic_path = load_or_create_traffic(
        path=traffic_path,
        requests=requests,
        clients=clients,
        days=days,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate IIS W3C training logs from production-like traffic")
    ...
    parser.add_argument("--clients", type=int, default=5, help="Client count when traffic must be generated.")
    parser.add_argument("--days", type=int, default=7, help="Traffic window when traffic must be generated.")
    args = parser.parse_args()
    result = generate_iis_logs(
